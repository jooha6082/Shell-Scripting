...
function generate_report {
  ...
  if [ $(ls ./reports_dir | wc -l) -eq $MAXIMUM_REPORTS ]; then
    oldest=$(ls -t ./reports_dir | tail -1)
    #rm -r ./reports_dir/$oldest
    #echo $MAXIMUM_REPORTS
    #echo $PWD
    #ls ./reports_dir -1t | tail -n +$MAXIMUM_REPORTS | xargs rm
    rm ./reports_dir/$oldest
  fi
...
function notify
{
  ...
  if [ "$cpu_usage_int" -gt "$CPU_THRESHOLD" ]; then
    report=$(ls ./reports_dir -Art | tail -n 1)
    /usr/bin/mailx -s "mail" $USER < ./reports_dir/$report
    echo "sent"
  fi
